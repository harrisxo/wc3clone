...
export function updateRankingsIfDue() {
  ...
  for (const user of users) {
    processGameJobs(user.id, user.race);
    const eco = accrueResources(user.id);
    ...
    const villages = territory.villages ?? 0,
      mines = territory.mines ?? 0,
      resourcePoints = Math.floor(eco.gold / 250 + eco.wood / 500);
    const points = Math.floor(mines * 10 + villages + supply + resourcePoints + upgradePoints);
    database
      .prepare(`INSERT INTO ranking_snapshots(user_id,points,villages,mines,unit_supply,resource_points,hero_points,upgrade_points,calculated_at) VALUES(?,?,?,?,?,?,0,?,?) ON CONFLICT(user_id) DO UPDATE SET points=excluded.points,villages=excluded.villages,mines=excluded.mines,unit_supply=excluded.unit_supply,resource_points=excluded.resource_points,upgrade_points=excluded.upgrade_points,calculated_at=excluded.calculated_at`)
      .run(user.id, points, villages, mines, supply, resourcePoints, upgradePoints, new Date(now).toISOString());
  }
}

export function getRanking() {
  updateRankingsIfDue();
  return database.prepare(`SELECT u.id,u.display_name AS username,u.race,r.points,r.villages,r.mines,r.unit_supply,r.resource_points,r.hero_points,r.upgrade_points,r.calculated_at,w.x home_x,w.y home_y FROM ranking_snapshots r JOIN users u ON u.id=r.user_id LEFT JOIN world_tiles w ON w.owner_user_id=u.id ORDER BY r.points DESC,u.id ASC`).all() as {
    id: number;
    username: string;
    race: Race;
    points: number;
