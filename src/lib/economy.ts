...
export type EconomyState = {
  ...
  goldWorkers: number;
  woodWorkers: number;
  goldWorkplaces: number;
  woodWorkplaces: number;
  busyWorkers: number;
  updatedAt: string;
};
...
export function accrueResources(userId: number): EconomyState {
  ...
  database.prepare(`UPDATE users SET gold = ?, wood = ?, gold_workers = ?, wood_workers = ?, resources_updated_at = ? WHERE id = ?`).run(state.gold, state.wood, state.goldWorkers, state.woodWorkers, state.updatedAt, userId);
  return state;
}
