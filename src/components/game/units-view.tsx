...
const typeCodes: Record<string, string> = { small: "KF", medium: "MF", goldmine: "GM" };

type OwnedTile = { x: number; y: number; field_type: string; is_main_village: number };
...
export function UnitsView({ state, home, race, ownedTiles, showAll }: { state: ReturnType<typeof getGameState>; home: { x: number; y: number }; race: Race; ownedTiles: OwnedTile[]; showAll: boolean }) {
  ...
  type LocationRow = { x: number; y: number; isHome: boolean; typeCode: string; typeName: string; quantities: Map<string, number> };
  ...
  const locationFor = (x: number, y: number, fieldType: string | null, isMainVillage: number | null) => {
    ...
    if (!row) {
      ...
      row = { x, y, isHome, typeCode, typeName, quantities: new Map() };
      locations.set(key, row);
    }
    return row;
  };
  locationFor(home.x, home.y, null, 1);
  if (showAll) for (const tile of ownedTiles) locationFor(tile.x, tile.y, tile.field_type, tile.is_main_village);
