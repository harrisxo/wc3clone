...
export function UnitsView({ state, home }: { state: ReturnType<typeof getGameState>; home: { x: number; y: number } }) {
  ...
  return <div className="units-view">
    ...
    <div className="army-table-wrap"><table className="game-table"><thead><tr><th>Feld</th><th>Typ</th>{combatUnits.map((unit) => <th title={unit.name} key={unit.key}>{unit.icon}</th>)}</tr></thead><tbody>
    ...
    <p className="hero-note">Helden erscheinen nur im Heldenturm.</p>
  </div>;
}