...
export function AttacksView({ marches, unitDefs }: { marches: March[]; unitDefs: UnitDefinition[] }) {
  ...
  return <div className="attacks-view">
    <div className="attacks-intro"><div><p className="section-kicker">Feldzüge</p><h2>Laufende Angriffe</h2><p>Deine Truppen sind unterwegs. Die Marschdauer richtet sich nach der Entfernung.</p></div><div className="attacks-count"><span>Unterwegs</span><strong>{marches.length}</strong></div></div>
    ...
    {marches.length === 0
      ...
      : <div className="attacks-list">{marches.map((march) => {
          ...
          const initialRemaining = Math.max(0, Math.ceil((new Date(march.arrivesAt).getTime() - Date.now()) / 1000));
          return <div className={`attack-row ${march.friendly ? "attack-move" : "attack-strike"}`} key={march.id}>
            <div className="attack-head">
              ...
              <div className="attack-route"><strong>{nameOf(march.sourceX, march.sourceY)}</strong><span aria-hidden="true">→</span><strong>{nameOf(march.targetX, march.targetY)}</strong></div>
            ...
            <div className="attack-body">
              ...
              <ul className="attack-units">{units.map((unit) => <li key={unit.name}>{unit.icon} {unit.name} <b>×{unit.quantity}</b></li>)}</ul>
