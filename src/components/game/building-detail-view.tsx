...
import { formatDomainDamage, researchDefs } from "@/lib/game-data";
      ...
      <div className="selected-unit-list">
        {availableUnits.filter((unit) => unit.role !== "hero").map((unit) => <form className="selected-unit" action={trainUnit} key={unit.key}>
          ...
          <button type="submit" name="quantity" value={1} className="selected-unit-trigger">
            ...
            <span className="selected-unit-info"><h4>{unit.name}</h4><small>{unit.supply} Nahrung · {Math.ceil(unit.seconds / 60)}m · ⚔ {formatDomainDamage(unit.damage)} · ▣ {unit.defense[0]}–{unit.defense[1]}</small><p>Gold {unit.gold} · Holz {unit.wood}</p></span>
          </button>
